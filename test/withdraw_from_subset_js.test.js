...
function verifyMerkleProof({ pathElements, pathIndices, leaf, root }) {
    pathElements.forEach((element, i) => {
        leaf = !pathIndices[i]
            ? poseidon([leaf, element])
            : poseidon([element, leaf]);
    });
    return leaf == root;
}
...
describe("withdraw_from_subset.circom (JS tests edition)", function () {
    before(async () => {
        ...
        this.deposit_tree = new MerkleTree({
            hasher: poseidon,
    ...
    it("should be able to generate proofs of inclusion in an allow subset", async () => {
        ...
        for (const path of paths) {
            try {
                const { pathIndices, pathElements } =
                    await this.allowlist_tree.path(path);
                expect(
                    verifyMerkleProof({
                        leaf,
                        root,
    ...
    it("should be able to generate valid zkps using the proof of inclusion in allowlist", async () => {
        ...
        console.log(
            chalk.green(
                "    The following zkp proof generation times include instantiating the deposit merkle tree and the allowlist tree"
            )
        );
        for (const path of paths) {
            try {
                ...
                const { pathElements: mainProof, pathRoot: root } =
                    await deposit_tree.path(path);
                const { pathElements: subsetProof, pathRoot: subsetRoot } =
                    await allowlist_tree.path(path);
    ...
    it("should be able to generate proofs of exclusion in a block subset", async () => {
        ...
        for (const path of paths) {
            try {
                const { pathIndices, pathElements } =
                    await this.blocklist_tree.path(path);
                expect(
                    verifyMerkleProof({
                        leaf,
                        root,
    ...
    it("should be able to generate valid zkps using the proof of exclusion in blocklist", async () => {
        ...
        console.log(
            chalk.blue(
                "    The following zkp proof generation times do not include instantiating the deposit merkle tree and the allowlist tree"
            )
        );
        for (const path of paths) {
            try {
